...
concept has_get = requires(U& u) {
  u.template get<InlineDemo::Payload::Words>();
};
...
int main() {
  ...
  seed.payload = InlineDemo::Payload::Words::Data{0x11223344U, 0x55667788U};
  seed.marker = static_cast<std::uint8_t>(0xAB);
  auto r = buf.emplace(seed);

  auto* words = r.payload.get_if<InlineDemo::Payload::Words>();
  ...
  assert(r.payload.holds_alternative<InlineDemo::Payload::Words>());
  assert(r.payload.index() == 0);
  ...
  auto* as_double = r.payload.get_if<double>();
  assert(as_double != nullptr);
  assert(static_cast<double>(*as_double) == 2.0);

  auto cr = static_cast<const noserde::Buffer<InlineDemo, 1>&>(buf)[0];
  auto* cdouble = cr.payload.get_if<double>();
  assert(cdouble != nullptr);
